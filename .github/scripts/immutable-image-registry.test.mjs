...
const OCI_SOURCE = 'https://github.com/DevPathAi/devpath-notification-svc';
const REPOSITORY = 'ghcr.io/devpathai/devpath-notification-svc';
...
test('404 requires the exact manifest endpoint and MANIFEST_UNKNOWN envelope', async (t) => {
  const mutations = [
    ...
    },
    {
      ...
      response: jsonResponse({
        errors: [{
          ...
          detail: { repository: 'devpath-notification-svc' },
        }],
      }, { status: 404 }),
      error: /MANIFEST_UNKNOWN/,
    },
    {
      ...
      url: 'https://ghcr.io/v2/devpathai/devpath-notification-svc/manifests/wrong',
      error: /response URL/,
    },
  ];
...
test('evidence binds the exact producer without credentials or mutable fields', async () => {
  ...
  const evidence = createImmutableImageEvidence(result, {
    repository: 'DevPathAi/devpath-notification-svc',
  ...
  assert.equal(evidence.repository, 'DevPathAi/devpath-notification-svc');
